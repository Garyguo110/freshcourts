const {Builder, By, WebDriverWait, until} = require('selenium-webdriver');
...
async function getSessionsForDate(dateString) {
    ...
    await _asyncForEach(courtDivList, async (courtDiv) => {
        let courtSessions = await _parseCourtAndSessionsFromDiv(courtDiv, dateString);
...
async function _parseCourtAndSessionsFromDiv(courtDiv, dateString) {
    let courtLink = await courtDiv.findElement(By.xpath('.//a[contains(@href,"spot")]'));
    let courtId = (await courtLink.getAttribute('href')).split('/').pop();
    let courtName = await courtLink.findElement(By.xpath('./span')).getAttribute('textContent');
...
async function _parseSessionDivList(court, courtDiv, dateString) {
    let courtSessions = [];
    try {
        let sessionDivList = await courtDiv.findElements(By.xpath('.//descendant::span[6]/div'));
...
async function _parseSessionsFromDiv(court, sessionDiv, dateString) {
    let timeSlot = await sessionDiv.findElement(By.xpath('.//descendant::span[1]')).getAttribute('textContent');
    let isAvailable = (await sessionDiv.findElements(By.className('p_AFDisabled'))).length == 0;
...
async function _getCourtDivList(driver) {
    ...
    return driver.wait(until.elementLocated(By.id(reservationTableId)),10000).findElements(By.xpath("./div"));
}
