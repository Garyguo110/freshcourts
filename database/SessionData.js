...
class SessionData {
    ...
    getDescription() {
        ...
        return `Session for court ${this.court.name} on ${this.date} at ${this.timeSlot} is ${availableText}`;
    }
}

function generateId(courtId, date, timeSlot) {
    return crypto.createHash('md5').update(courtId + date + timeSlot).digest('hex');
}
